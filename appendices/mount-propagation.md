...
# [マウントはなぜ伝播するのか](#why-mount-propagates) {#why-mount-propagates}

## [はじめに](#introduction) {#introduction}

[06-namespace](../../06-namespace/) の「Mount namespace」で、プロセスごとに異なるファイルシステムビューを持てることを学びました
...
- [マウント伝播とは](#what-is-mount-propagation)
- [伝播モード](#propagation-modes)
- [確認方法](#confirmation-methods)
- [設定方法](#setting-methods)
- [コンテナでの活用](#container-usage)
- [まとめ](#summary)
- [参考資料](#references)

---

## [マウント伝播とは](#what-is-mount-propagation) {#what-is-mount-propagation}

### [基本的な概念](#basic-concept) {#basic-concept}
...
## [伝播モード](#propagation-modes) {#propagation-modes}

### [ピアグループとは（先に理解すべき概念）](#peer-group) {#peer-group}
...
### [shared（共有）](#shared-mode) {#shared-mode}
...
### [private（プライベート）](#private-mode) {#private-mode}
...
### [slave（スレーブ）](#slave-mode) {#slave-mode}
...
### [unbindable（バインド不可）](#unbindable-mode) {#unbindable-mode}
...
### [/proc/self/mountinfo](#proc-self-mountinfo) {#proc-self-mountinfo}
...
### [フィールドの読み方](#reading-fields) {#reading-fields}
...
### [findmnt コマンド](#findmnt-command) {#findmnt-command}
...
## [設定方法](#setting-methods) {#setting-methods}

### [mount コマンドで設定](#setting-with-mount-command) {#setting-with-mount-command}
...
### [再帰的に設定（r- prefix）](#recursive-setting) {#recursive-setting}
...
### [マウント時の設定 vs 既存マウントへの設定](#setting-at-mount-vs-existing) {#setting-at-mount-vs-existing}
...
## [コンテナでの活用](#container-usage) {#container-usage}

### [Docker のボリュームマウント](#docker-volume-mount) {#docker-volume-mount}
...
### [モードの選び方](#choosing-mode) {#choosing-mode}
...
### [例：ホストの USB デバイスをコンテナで使う](#example-usb-in-container) {#example-usb-in-container}
...
### [Kubernetes の mountPropagation](#kubernetes-mount-propagation) {#kubernetes-mount-propagation}
...
- [Shared Subtrees - kernel.org](https://www.kernel.org/doc/Documentation/filesystems/sharedsubtree.txt){:target="\_blank"}
  - マウント伝播の詳細仕様
