---
layout: default
title: psのSTAT列は何を意味するのか
---

# [psのSTAT列は何を意味するのか](#what-ps-stat-column-means) {#what-ps-stat-column-means}

## [はじめに](#introduction) {#introduction}

[04-process-management](../../04-process-management/) の「プロセスの状態」で、プロセスには R、S、D などの状態があることを学びました
...
- [STAT列の構造](#stat-column-structure)
- [主要な状態コード](#main-state-codes)
- [追加フラグ](#additional-flags)
- [D状態の危険性](#d-state-danger)
- [ゾンビプロセスの対処](#handling-zombie-processes)
- [状態の確認方法](#checking-states)
- [まとめ](#summary)
- [参考資料](#references)

---

## [STAT列の構造](#stat-column-structure) {#stat-column-structure}

### [基本構造](#basic-structure) {#basic-structure}
...
### [最初の文字が状態コード](#first-char-is-state-code) {#first-char-is-state-code}
...
### [2文字目以降が追加フラグ](#second-char-flags) {#second-char-flags}
...
### [R（Running / Runnable）](#r-running-runnable) {#r-running-runnable}
...
### [S（Sleeping - Interruptible）](#s-sleeping-interruptible) {#s-sleeping-interruptible}
...
### [D（Sleeping - Uninterruptible）](#d-sleeping-uninterruptible) {#d-sleeping-uninterruptible}
...
### [t（Tracing stop）](#t-tracing-stop) {#t-tracing-stop}
...
## [追加フラグ](#additional-flags) {#additional-flags}
...
### [組み合わせ例](#combination-examples) {#combination-examples}
...
## [D状態の危険性](#d-state-danger) {#d-state-danger}

### [なぜD状態は特別か](#why-d-state-is-special) {#why-d-state-is-special}
...
### [カーネル内部での実装](#kernel-internal-implementation) {#kernel-internal-implementation}
...
### [D状態の正常なケース](#normal-d-state-cases) {#normal-d-state-cases}
...
### [D状態が長時間続く場合](#prolonged-d-state) {#prolonged-d-state}
...
### [D状態プロセスの影響](#d-state-process-impact) {#d-state-process-impact}

D 状態のプロセスは<strong>ロードアベレージに加算</strong>されます

<strong>ロードアベレージとは</strong>

ロードアベレージは「実行を待っているプロセスの数」の平均です（詳細は [load-average.md](../load-average/) を参照）
...
### [D状態の調査方法](#investigating-d-state) {#investigating-d-state}
...
## [ゾンビプロセスの対処](#handling-zombie-processes) {#handling-zombie-processes}

### [ゾンビとは](#what-is-zombie) {#what-is-zombie}
...
### [なぜゾンビは存在するのか](#why-zombies-exist) {#why-zombies-exist}
...
### [ゾンビの問題](#zombie-problems) {#zombie-problems}
...
### [ゾンビの対処法](#zombie-remedies) {#zombie-remedies}
...
## [状態の確認方法](#checking-states) {#checking-states}
...
### [/proc/\[pid\]/stat](#proc-pid-stat) {#proc-pid-stat}
...
### [/proc/\[pid\]/status](#proc-pid-status) {#proc-pid-status}
...
- [ps(1) - Linux manual page](https://man7.org/linux/man-pages/man1/ps.1.html){:target="\_blank"}
  - STAT 列の詳細
