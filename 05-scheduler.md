...
# [05-scheduler：CPU 時間の配分](#scheduler) {#scheduler}

## [はじめに](#introduction) {#introduction}

[04-process-management](../04-process-management/) では、カーネルがプロセスを管理する仕組みを学びました
...
1. [スケジューラとは何か](#what-is-scheduler)
2. [スケジューリングポリシー](#scheduling-policies)
3. [nice 値と優先度](#nice-value-and-priority)
4. [CFS（Completely Fair Scheduler）](#cfs)
5. [リアルタイムスケジューリング](#realtime-scheduling)
6. [/proc でスケジューリング情報を観察](#observing-scheduling-info)
7. [CPU アフィニティ](#cpu-affinity)
8. [カーネルパラメータ](#kernel-parameters)
9. [次のステップ](#next-steps)
10. [用語集](#glossary)
11. [参考資料](#references)
...
## [スケジューラとは何か](#what-is-scheduler) {#what-is-scheduler}

### [なぜスケジューラが必要か](#why-scheduler-needed) {#why-scheduler-needed}
...
### [スケジューラの役割](#scheduler-role) {#scheduler-role}
...
### [タイムシェアリング](#time-sharing) {#time-sharing}
...
### [プリエンプション](#preemption) {#preemption}
...
## [スケジューリングポリシー](#scheduling-policies) {#scheduling-policies}

### [ポリシーとは](#what-is-policy) {#what-is-policy}
...
### [6 つのスケジューリングポリシー](#six-scheduling-policies) {#six-scheduling-policies}
...
### [通常ポリシー vs リアルタイムポリシー](#normal-vs-realtime-policy) {#normal-vs-realtime-policy}
...
### [現在のポリシーを確認する](#checking-current-policy) {#checking-current-policy}
...
### [利用可能なポリシーを確認する](#checking-available-policies) {#checking-available-policies}
...
## [nice 値と優先度](#nice-value-and-priority) {#nice-value-and-priority}

### [nice 値とは](#what-is-nice-value) {#what-is-nice-value}
...
### [nice 値の範囲](#nice-value-range) {#nice-value-range}
...
### [nice 値と CPU 時間の関係](#nice-value-and-cpu-time) {#nice-value-and-cpu-time}
...
### [nice 値の確認方法](#checking-nice-value) {#checking-nice-value}
...
### [nice 値の変更方法](#changing-nice-value) {#changing-nice-value}
...
### [静的優先度と動的優先度](#static-and-dynamic-priority) {#static-and-dynamic-priority}
...
## [CFS（Completely Fair Scheduler）](#cfs) {#cfs}

### [CFS とは](#what-is-cfs) {#what-is-cfs}
...
### [なぜ CFS が必要だったのか：歴史的背景](#why-cfs-historical) {#why-cfs-historical}
...
### [CFS の基本概念](#cfs-basic-concept) {#cfs-basic-concept}
...
### [nice 値と vruntime](#nice-value-and-vruntime) {#nice-value-and-vruntime}
...
## [リアルタイムスケジューリング](#realtime-scheduling) {#realtime-scheduling}

### [リアルタイムとは](#what-is-realtime) {#what-is-realtime}
...
### [なぜリアルタイムが必要なのか：音声処理の例](#why-realtime-audio-example) {#why-realtime-audio-example}
...
### [SCHED_FIFO](#sched-fifo) {#sched-fifo}
...
### [リアルタイム優先度](#realtime-priority) {#realtime-priority}
...
### [リアルタイムプロセスの設定](#realtime-process-configuration) {#realtime-process-configuration}
...
### [RT スロットリング](#rt-throttling) {#rt-throttling}
...
## [/proc でスケジューリング情報を観察](#observing-scheduling-info) {#observing-scheduling-info}

### [/proc/\[pid\]/sched](#proc-pid-sched) {#proc-pid-sched}
...
### [/proc/\[pid\]/schedstat](#proc-pid-schedstat) {#proc-pid-schedstat}
...
### [/proc/\[pid\]/stat のスケジューリング関連フィールド](#proc-pid-stat-scheduling-fields) {#proc-pid-stat-scheduling-fields}
...
### [/proc/stat の CPU 時間](#proc-stat-cpu-time) {#proc-stat-cpu-time}
...
### [ps コマンドでスケジューリング情報を確認](#checking-scheduling-with-ps) {#checking-scheduling-with-ps}
...
## [CPU アフィニティ](#cpu-affinity) {#cpu-affinity}

### [CPU アフィニティとは](#what-is-cpu-affinity) {#what-is-cpu-affinity}
...
### [なぜ CPU アフィニティを使うのか](#why-cpu-affinity) {#why-cpu-affinity}
...
### [CPU アフィニティの確認](#checking-cpu-affinity) {#checking-cpu-affinity}
...
### [CPU アフィニティの設定](#setting-cpu-affinity) {#setting-cpu-affinity}
...
### [システムの CPU 情報](#system-cpu-info) {#system-cpu-info}
...
## [カーネルパラメータ](#kernel-parameters) {#kernel-parameters}

### [スケジューラ関連のカーネルパラメータ](#scheduler-kernel-parameters) {#scheduler-kernel-parameters}

`/proc/sys/kernel/` 以下にスケジューラの動作を調整するパラメータがあります

### [RT スロットリングパラメータ](#rt-throttling-parameters) {#rt-throttling-parameters}
...
### [SCHED_RR タイムスライス](#sched-rr-timeslice) {#sched-rr-timeslice}
...
### [その他のパラメータ](#other-parameters) {#other-parameters}
...
次の [06-namespace](../06-namespace/) では、プロセスを「隔離」する仕組みを学びます
...
- [sched(7) - Linux manual page](https://man7.org/linux/man-pages/man7/sched.7.html){:target="\_blank"}
  - スケジューリングポリシーと優先度の概要

<strong>nice 値と優先度</strong>

- [nice(2) - Linux manual page](https://man7.org/linux/man-pages/man2/nice.2.html){:target="\_blank"}
  - nice 値の変更
- [setpriority(2) - Linux manual page](https://man7.org/linux/man-pages/man2/setpriority.2.html){:target="\_blank"}
  - 優先度の取得と設定
- [getpriority(2) - Linux manual page](https://man7.org/linux/man-pages/man2/getpriority.2.html){:target="\_blank"}
  - 優先度の取得

<strong>スケジューリングポリシーの設定</strong>

- [sched_setscheduler(2) - Linux manual page](https://man7.org/linux/man-pages/man2/sched_setscheduler.2.html){:target="\_blank"}
  - スケジューリングポリシーの設定
- [sched_getscheduler(2) - Linux manual page](https://man7.org/linux/man-pages/man2/sched_getscheduler.2.html){:target="\_blank"}
  - スケジューリングポリシーの取得

<strong>CPU アフィニティ</strong>

- [sched_setaffinity(2) - Linux manual page](https://man7.org/linux/man-pages/man2/sched_setaffinity.2.html){:target="\_blank"}
  - CPU アフィニティの設定
- [sched_getaffinity(2) - Linux manual page](https://man7.org/linux/man-pages/man2/sched_getaffinity.2.html){:target="\_blank"}
  - CPU アフィニティの取得
