...
# [04-process-management：カーネルのプロセス管理](#process-management) {#process-management}

## [はじめに](#introduction) {#introduction}

[03-virtual-memory](../03-virtual-memory/) では、カーネルがプロセスごとに独立したメモリ空間を提供する仕組みを学びました
...
1. [カーネルがプロセスについて管理する情報](#process-descriptor-info)
2. [プロセスの状態](#process-states)
3. [/proc/[pid]/ でプロセスを観察する](#observing-process-with-proc)
4. [プロセスの資格情報](#process-credentials)
5. [プロセスグループとセッション](#process-group-and-session)
6. [制御端末](#controlling-terminal)
7. [デーモンプロセス](#daemon-process)
8. [プロセスの作成（カーネル視点）](#process-creation-kernel-view)
9. [プロセスの終了とゾンビ](#process-exit-and-zombie)
...
## [カーネルがプロセスについて管理する情報](#process-descriptor-info) {#process-descriptor-info}

### [プロセス記述子とは](#what-is-process-descriptor) {#what-is-process-descriptor}
...
### [管理される情報のカテゴリ](#managed-info-categories) {#managed-info-categories}
...
### [/proc/self/status で確認する](#checking-proc-self-status) {#checking-proc-self-status}
...
### [状態とは何か](#what-is-state) {#what-is-state}

プロセスは常にいずれかの<strong>状態</strong>（state）にあります

カーネルはこの状態を追跡し、適切なタイミングで CPU を割り当てます

### [主要な状態コード](#main-state-codes) {#main-state-codes}
...
### [状態の観察方法](#observing-states) {#observing-states}
...
### [D 状態（Uninterruptible Sleep）の意味](#d-state-meaning) {#d-state-meaning}
...
D 状態の詳しい実装については [appendices/process-state.md](../appendices/process-state/) で学べます
...
詳しくは [appendices/load-average.md](../appendices/load-average/) で学べます

### [Z 状態（ゾンビ）の意味](#z-state-meaning) {#z-state-meaning}
...
## [/proc/\[pid\]/ でプロセスを観察する](#observing-process-with-proc) {#observing-process-with-proc}

### [/proc/\[pid\]/ とは](#what-is-proc-pid) {#what-is-proc-pid}
...
### [/proc/\[pid\]/status の詳細](#proc-pid-status-details) {#proc-pid-status-details}
...
### [/proc/\[pid\]/stat の詳細](#proc-pid-stat-details) {#proc-pid-stat-details}
...
### [/proc/\[pid\]/fd/ でファイルディスクリプタを確認](#checking-fd-with-proc) {#checking-fd-with-proc}
...
## [プロセスの資格情報](#process-credentials) {#process-credentials}

### [資格情報とは](#what-is-credentials) {#what-is-credentials}
...
### [GID の種類](#gid-types) {#gid-types}

GID も UID と同様に、実、実効、保存、ファイルシステムの 4 種類があります

### [補助グループ](#supplementary-groups) {#supplementary-groups}
...
### [ケーパビリティ](#capabilities) {#capabilities}
...
## [プロセスグループとセッション](#process-group-and-session) {#process-group-and-session}

### [なぜプロセスグループとセッションが必要なのか](#why-process-group-and-session) {#why-process-group-and-session}
...
### [シェルのジョブ制御との関係](#job-control-relationship) {#job-control-relationship}
...
### [プロセスグループとは](#what-is-process-group) {#what-is-process-group}
...
### [セッションとは](#what-is-session) {#what-is-session}
...
### [観察方法](#observation-method) {#observation-method}
...
### [シグナルとプロセスグループ](#signal-and-process-group) {#signal-and-process-group}
...
## [制御端末](#controlling-terminal) {#controlling-terminal}

### [制御端末とは](#what-is-controlling-terminal) {#what-is-controlling-terminal}

<strong>制御端末</strong>（controlling terminal）は、セッションに関連付けられた端末です

ターミナルから起動したプロセスは、通常そのターミナルを制御端末として持ちます

### [制御端末の役割](#role-of-controlling-terminal) {#role-of-controlling-terminal}
...
### [制御端末の確認](#checking-controlling-terminal) {#checking-controlling-terminal}
...
### [制御端末を持たないプロセス](#processes-without-terminal) {#processes-without-terminal}
...
## [デーモンプロセス](#daemon-process) {#daemon-process}

### [デーモンとは](#what-is-daemon) {#what-is-daemon}
...
### [なぜ特別な作成手順が必要か](#why-special-creation) {#why-special-creation}
...
### [従来のデーモン作成手順](#traditional-daemon-creation) {#traditional-daemon-creation}
...
### [一般的なアプローチ：systemd](#systemd-approach) {#systemd-approach}
...
### [デーモンの観察](#observing-daemon) {#observing-daemon}
...
## [プロセスの作成（カーネル視点）](#process-creation-kernel-view) {#process-creation-kernel-view}

### [fork() のカーネル側処理](#fork-kernel-processing) {#fork-kernel-processing}
...
### [fork() と clone()](#fork-and-clone) {#fork-and-clone}
...
### [clone() の主要なフラグ](#clone-main-flags) {#clone-main-flags}
...
### [スレッドとプロセスの違い](#thread-vs-process) {#thread-vs-process}
...
### [スレッドの観察](#observing-threads) {#observing-threads}
...
## [プロセスの終了とゾンビ](#process-exit-and-zombie) {#process-exit-and-zombie}

### [exit() のカーネル側処理](#exit-kernel-processing) {#exit-kernel-processing}
...
### [ゾンビはなぜ存在するのか](#why-zombie-exists) {#why-zombie-exists}
...
### [ゾンビの問題](#zombie-problem) {#zombie-problem}
...
### [孤児プロセスと init](#orphan-and-init) {#orphan-and-init}
...
### [ゾンビの観察](#observing-zombie) {#observing-zombie}
...
次の [05-scheduler](../05-scheduler/) では、CPU という限られたリソースをプロセスに配分する「スケジューラ」の仕組みを学びます
...
- [credentials(7) - Linux manual page](https://man7.org/linux/man-pages/man7/credentials.7.html){:target="\_blank"}
  - プロセス識別子、資格情報、グループ
...
- [setsid(2) - Linux manual page](https://man7.org/linux/man-pages/man2/setsid.2.html){:target="\_blank"}
  - 新しいセッションの作成
- [setpgid(2) - Linux manual page](https://man7.org/linux/man-pages/man2/setpgid.2.html){:target="\_blank"}
  - プロセスグループ ID の設定

<strong>デーモンプロセス</strong>

- [daemon(7) - Linux manual page](https://man7.org/linux/man-pages/man7/daemon.7.html){:target="\_blank"}
  - デーモンプログラムの書き方

<strong>プロセス終了</strong>

- [exit(2) - Linux manual page](https://man7.org/linux/man-pages/man2/exit.2.html){:target="\_blank"}
  - プロセスの終了
