...
# [01-kernel：カーネルとは何か](#kernel) {#kernel}
...
1. [カーネルとは何か](#what-is-kernel)
2. [カーネル空間とユーザー空間](#kernel-space-and-user-space)
3. [カーネルが提供する主要な機能](#kernel-main-features)
4. [カーネルの情報を取得する](#getting-kernel-information)
5. [/proc ファイルシステム](#proc-filesystem)
6. [/sys ファイルシステム](#sys-filesystem)
7. [カーネルモジュール](#kernel-modules)
8. [次のステップ](#next-steps)
9. [用語集](#glossary)
10. [参考資料](#references)
...
## [カーネルとは何か](#what-is-kernel) {#what-is-kernel}
...
### [カーネルがないとどうなるか](#without-kernel) {#without-kernel}
...
## [カーネル空間とユーザー空間](#kernel-space-and-user-space) {#kernel-space-and-user-space}

### [なぜ2つの空間に分かれているのか](#why-two-spaces) {#why-two-spaces}
...
### [セキュリティと安定性](#security-and-stability) {#security-and-stability}
...
### [システムコールによる橋渡し](#bridging-via-syscall) {#bridging-via-syscall}
...
システムコールの詳しい仕組みは、次のトピック [02-syscall](../02-syscall/) で学びます

---

## [カーネルが提供する主要な機能](#kernel-main-features) {#kernel-main-features}

カーネルは、以下の主要な機能を提供しています

### [1. プロセス管理](#process-management) {#process-management}
...
### [2. メモリ管理](#memory-management) {#memory-management}
...
### [3. ファイルシステム](#filesystem) {#filesystem}
...
### [4. デバイス管理](#device-management) {#device-management}
...
## [カーネルの情報を取得する](#getting-kernel-information) {#getting-kernel-information}

カーネルに関する情報を取得するには、いくつかの方法があります

### [uname() システムコール](#uname-syscall) {#uname-syscall}
...
### [sysinfo() システムコール](#sysinfo-syscall) {#sysinfo-syscall}
...
スワップの詳しい仕組みは [appendices/swap.md](../appendices/swap/) で学べます
...
ロードアベレージの詳しい計算方法や解釈は [appendices/load-average.md](../appendices/load-average/) で学べます
...
### [gethostname() システムコール](#gethostname-syscall) {#gethostname-syscall}
...
### [/proc の特徴](#proc-characteristics) {#proc-characteristics}
...
### [/proc の主要なファイル・ディレクトリ](#proc-main-files) {#proc-main-files}
...
### [/proc の使い方](#proc-usage) {#proc-usage}
...
## [/sys ファイルシステム](#sys-filesystem) {#sys-filesystem}

### [/sys とは](#what-is-sys) {#what-is-sys}
...
### [/sys の主要なディレクトリ](#sys-main-directories) {#sys-main-directories}
...
cgroup の詳細は [07-cgroup](../07-cgroup/) で学びます

---

## [カーネルモジュール](#kernel-modules) {#kernel-modules}

### [カーネルモジュールとは](#what-is-kernel-module) {#what-is-kernel-module}
...
### [なぜカーネルモジュールがあるのか](#why-kernel-modules) {#why-kernel-modules}
...
### [カーネルモジュールの確認](#checking-kernel-modules) {#checking-kernel-modules}
...
次の [02-syscall](../02-syscall/) では、ユーザー空間からカーネルに処理を依頼する「システムコール」の仕組みを詳しく学びます
...
- [uname(2) - Linux manual page](https://man7.org/linux/man-pages/man2/uname.2.html){:target="\_blank"}
  - システム情報の取得
- [sysinfo(2) - Linux manual page](https://man7.org/linux/man-pages/man2/sysinfo.2.html){:target="\_blank"}
  - メモリやプロセスの統計情報
- [gethostname(2) - Linux manual page](https://man7.org/linux/man-pages/man2/gethostname.2.html){:target="\_blank"}
  - ホスト名の取得
...
- [proc(5) - Linux manual page](https://man7.org/linux/man-pages/man5/proc.5.html){:target="\_blank"}
  - /proc ファイルシステム
- [sysfs(5) - Linux manual page](https://man7.org/linux/man-pages/man5/sysfs.5.html){:target="\_blank"}
  - /sys ファイルシステム

<strong>カーネルドキュメント</strong>

- [The Linux Kernel documentation](https://www.kernel.org/doc/html/latest/){:target="\_blank"}
  - カーネルの公式ドキュメント
