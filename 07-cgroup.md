...
# [07-cgroup：リソースの制限](#cgroup) {#cgroup}

## [はじめに](#introduction) {#introduction}

[06-namespace](../06-namespace/) では、プロセスを隔離する namespace の仕組みを学びました
...
1. [cgroup とは何か](#what-is-cgroup)
2. [cgroup v1 と cgroup v2](#cgroup-v1-and-v2)
3. [主要なコントローラ](#main-controllers)
4. [CPU の制限](#cpu-limit)
5. [メモリの制限](#memory-limit)
6. [I/O の制限](#io-limit)
7. [PID の制限](#pid-limit)
8. [/sys/fs/cgroup での cgroup 観察](#observing-cgroup)
9. [プロセスの cgroup 所属](#process-cgroup-membership)
10. [cgroup の操作](#cgroup-operations)
11. [コンテナ技術との関係](#container-technology-relationship)
12. [まとめ：このリポジトリで学んだこと](#summary)
13. [用語集](#glossary)
14. [参考資料](#references)

---

## [cgroup とは何か](#what-is-cgroup) {#what-is-cgroup}

### [リソース制限の必要性](#resource-limit-necessity) {#resource-limit-necessity}
...
### [cgroup の役割](#cgroup-role) {#cgroup-role}
...
### [cgroup の特徴](#cgroup-characteristics) {#cgroup-characteristics}
...
### [cgroup vs ulimit](#cgroup-vs-ulimit) {#cgroup-vs-ulimit}
...
## [cgroup v1 と cgroup v2](#cgroup-v1-and-v2) {#cgroup-v1-and-v2}

### [cgroup の歴史](#cgroup-history) {#cgroup-history}
...
### [cgroup v1 の特徴](#cgroup-v1-characteristics) {#cgroup-v1-characteristics}
...
### [cgroup v2 の特徴](#cgroup-v2-characteristics) {#cgroup-v2-characteristics}
...
### [v1 と v2 の比較](#v1-v2-comparison) {#v1-v2-comparison}
...
### [No Internal Processes ルール](#no-internal-processes-rule) {#no-internal-processes-rule}
...
### [現在の推奨](#current-recommendation) {#current-recommendation}
...
### [利用可能なコントローラの確認](#checking-available-controllers) {#checking-available-controllers}
...
### [子 cgroup で有効なコントローラ](#enabled-controllers-in-child) {#enabled-controllers-in-child}
...
## [CPU の制限](#cpu-limit) {#cpu-limit}

### [CPU コントローラの役割](#cpu-controller-role) {#cpu-controller-role}
...
### [cpu.weight による配分](#cpu-weight-distribution) {#cpu-weight-distribution}
...
### [cpu.max による帯域幅制限](#cpu-max-bandwidth-limit) {#cpu-max-bandwidth-limit}
...
### [CPU 統計の確認](#checking-cpu-stats) {#checking-cpu-stats}
...
### [weight と max の使い分け](#weight-vs-max) {#weight-vs-max}
...
## [メモリの制限](#memory-limit) {#memory-limit}

### [memory コントローラの役割](#memory-controller-role) {#memory-controller-role}
...
### [memory.max（ハード制限）](#memory-max-hard-limit) {#memory-max-hard-limit}
...
詳細は [OOM キラーの仕組み](../appendices/oom/) を参照してください

### [memory.high（ソフト制限）](#memory-high-soft-limit) {#memory-high-soft-limit}
...
### [memory.low と memory.min（保護）](#memory-low-and-min-protection) {#memory-low-and-min-protection}
...
### [制限の階層](#limit-hierarchy) {#limit-hierarchy}
...
### [メモリ使用量の確認](#checking-memory-usage) {#checking-memory-usage}
...
詳細は [03-virtual-memory](../03-virtual-memory/) を参照してください
...
## [I/O の制限](#io-limit) {#io-limit}

### [io コントローラの役割](#io-controller-role) {#io-controller-role}
...
### [io.weight による配分](#io-weight-distribution) {#io-weight-distribution}
...
### [io.max による帯域幅制限](#io-max-bandwidth-limit) {#io-max-bandwidth-limit}
...
### [デバイスのメジャー番号とマイナー番号の確認](#device-major-minor-numbers) {#device-major-minor-numbers}
...
### [I/O 統計の確認](#checking-io-stats) {#checking-io-stats}
...
## [PID の制限](#pid-limit) {#pid-limit}

### [pids コントローラの役割](#pids-controller-role) {#pids-controller-role}

<strong>pids コントローラ</strong>は、プロセスグループが作成できるプロセス数を制限します

これは<strong>fork bomb</strong>（無限にプロセスを生成する攻撃）への対策として重要です

### [pids.max の設定](#setting-pids-max) {#setting-pids-max}
...
### [現在のプロセス数の確認](#checking-current-process-count) {#checking-current-process-count}

```bash
cat /sys/fs/cgroup/group_a/pids.current
```

### [fork bomb 対策](#fork-bomb-countermeasure) {#fork-bomb-countermeasure}
...
## [/sys/fs/cgroup での cgroup 観察](#observing-cgroup) {#observing-cgroup}

### [cgroup ファイルシステム](#cgroup-filesystem) {#cgroup-filesystem}
...
### [主要なファイルの役割](#main-file-roles) {#main-file-roles}
...
### [cgroup.events の確認](#checking-cgroup-events) {#checking-cgroup-events}
...
### [実際の cgroup 構造を確認](#checking-actual-cgroup-structure) {#checking-actual-cgroup-structure}
...
## [プロセスの cgroup 所属](#process-cgroup-membership) {#process-cgroup-membership}

### [/proc/\[pid\]/cgroup](#proc-pid-cgroup) {#proc-pid-cgroup}
...
### [形式の説明](#format-explanation) {#format-explanation}
...
### [複数プロセスの cgroup を比較](#comparing-process-cgroups) {#comparing-process-cgroups}
...
### [Docker コンテナの場合](#docker-container-case) {#docker-container-case}
...
## [cgroup の操作](#cgroup-operations) {#cgroup-operations}

### [cgroup の作成](#creating-cgroup) {#creating-cgroup}
...
### [コントローラの有効化](#enabling-controllers) {#enabling-controllers}
...
### [プロセスの移動](#moving-processes) {#moving-processes}
...
### [リソース制限の設定](#setting-resource-limits) {#setting-resource-limits}
...
### [cgroup の削除](#deleting-cgroup) {#deleting-cgroup}
...
## [コンテナ技術との関係](#container-technology-relationship) {#container-technology-relationship}

### [namespace と cgroup の組み合わせ](#namespace-and-cgroup-combination) {#namespace-and-cgroup-combination}
...
### [コンテナが使用する主な技術](#container-main-technologies) {#container-main-technologies}
...
### [Docker でのリソース制限](#docker-resource-limits) {#docker-resource-limits}
...
### [Docker コンテナの cgroup を確認](#checking-docker-container-cgroup) {#checking-docker-container-cgroup}
...
### [Kubernetes でのリソース制限](#kubernetes-resource-limits) {#kubernetes-resource-limits}
...
## [まとめ：このリポジトリで学んだこと](#summary) {#summary}
...
- [Control Group v2 - The Linux Kernel documentation](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v2.html){:target="\_blank"}
...
- [Memory Resource Controller - The Linux Kernel documentation](https://www.kernel.org/doc/html/latest/admin-guide/cgroup-v1/memory.html){:target="\_blank"}
  - メモリコントローラの詳細
- [CPU Controller - The Linux Kernel documentation](https://www.kernel.org/doc/html/latest/scheduler/sched-bwc.html){:target="\_blank"}
  - CPU 帯域幅制御の詳細
