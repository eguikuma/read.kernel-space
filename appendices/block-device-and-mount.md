---
layout: default
title: ディスクはどうやってファイルになるのか
---

# [ディスクはどうやってファイルになるのか](#how-disk-becomes-file) {#how-disk-becomes-file}

## [はじめに](#introduction) {#introduction}

[filesystem](../filesystem/) で、ファイルの正体は inode であり、データはブロック単位でディスクに保存されると学びました
...
- [ブロックデバイスとは](#what-is-block-device)
- [「空のブロックデバイス」とは](#what-is-empty-block-device)
- [ファイルシステムの作成（mkfs）](#creating-filesystem)
- [マウント：ディレクトリツリーへの接続](#mount-to-directory-tree)
- [なぜ3層に分離されているのか](#why-three-layers)
- [ファイルシステムのシグネチャとwipefs](#filesystem-signature-and-wipefs)
...
## [ブロックデバイスとは](#what-is-block-device) {#what-is-block-device}

### [デバイスの2つの種類](#two-types-of-devices) {#two-types-of-devices}
...
### [なぜ「かたまり」単位なのか](#why-block-units) {#why-block-units}
...
### [メモリのページサイズとの関係](#relationship-with-page-size) {#relationship-with-page-size}
...
詳しくは [page](../page/) の「なぜ4KBなのか」を参照してください

### [Linux での表現](#linux-representation) {#linux-representation}
...
### [ブロックデバイスのイメージ](#block-device-image) {#block-device-image}
...
## [「空のブロックデバイス」とは](#what-is-empty-block-device) {#what-is-empty-block-device}

### [「空」の意味](#meaning-of-empty) {#meaning-of-empty}
...
### [ループバックデバイス](#loopback-device) {#loopback-device}
...
## [ファイルシステムの作成（mkfs）](#creating-filesystem) {#creating-filesystem}

### [mkfs とは](#what-is-mkfs) {#what-is-mkfs}
...
### [mkfs が何をするか](#what-mkfs-does) {#what-mkfs-does}
...
inode の詳細は [filesystem](../filesystem/) の「ファイルの正体：inode」を参照してください
...
### [mkfs は「フォーマット」](#mkfs-format) {#mkfs-format}
...
## [マウント：ディレクトリツリーへの接続](#mount-to-directory-tree) {#mount-to-directory-tree}

### [なぜマウントが必要なのか](#why-mount-needed) {#why-mount-needed}
...
### [マウント前の状態](#state-before-mount) {#state-before-mount}
...
### [mount コマンドの実行](#executing-mount-command) {#executing-mount-command}
...
マウントの詳細（Mount namespace やコンテナとの関係）は [06-namespace](../../06-namespace/) を参照してください

---

## [なぜ3層に分離されているのか](#why-three-layers) {#why-three-layers}

### [Windows も同じ仕組みを使っている](#windows-same-mechanism) {#windows-same-mechanism}
...
### [分離されていることのメリット](#benefits-of-separation) {#benefits-of-separation}
...
VFS（仮想ファイルシステム）がこの多様性を支えています（詳細は [filesystem](../filesystem/) の「VFS：なぜ抽象化が必要なのか」を参照）

<strong>マウントが独立</strong>

同じファイルシステムを異なる場所にマウントしたり、読み取り専用でマウントしたりできます

Mount namespace は、この柔軟性を活かしてコンテナごとに異なるファイルシステムの見え方を実現しています（詳細は [06-namespace](../../06-namespace/) を参照）

### [もし分離されていなかったら](#if-not-separated) {#if-not-separated}
...
## [ファイルシステムのシグネチャとwipefs](#filesystem-signature-and-wipefs) {#filesystem-signature-and-wipefs}

### [シグネチャとは](#what-is-signature) {#what-is-signature}
...
### [wipefs コマンド](#wipefs-command) {#wipefs-command}
...
### [マウント中に wipefs を実行するとどうなるか](#wipefs-while-mounted) {#wipefs-while-mounted}
...
### [ブロックデバイスの一覧](#listing-block-devices) {#listing-block-devices}
...
`/sys/block/` でもブロックデバイスの一覧を確認できます（詳細は [sys](../sys/) を参照）

### [ファイルシステムのシグネチャ](#filesystem-signature) {#filesystem-signature}
...
### [マウント状態](#mount-status) {#mount-status}
...
- [lsblk(8) - Linux manual page](https://man7.org/linux/man-pages/man8/lsblk.8.html){:target="\_blank"}
  - ブロックデバイスの一覧表示
- [mkfs(8) - Linux manual page](https://man7.org/linux/man-pages/man8/mkfs.8.html){:target="\_blank"}
  - ファイルシステムの作成
- [mount(8) - Linux manual page](https://man7.org/linux/man-pages/man8/mount.8.html){:target="\_blank"}
  - ファイルシステムのマウント
- [mount(2) - Linux manual page](https://man7.org/linux/man-pages/man2/mount.2.html){:target="\_blank"}
  - マウントのシステムコール
- [loop(4) - Linux manual page](https://man7.org/linux/man-pages/man4/loop.4.html){:target="\_blank"}
  - ループバックデバイス
- [wipefs(8) - Linux manual page](https://man7.org/linux/man-pages/man8/wipefs.8.html){:target="\_blank"}
  - ファイルシステムシグネチャの消去
