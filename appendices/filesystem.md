...
# [ファイルとは本当は何なのか](#what-is-a-file) {#what-is-a-file}

## [はじめに](#introduction) {#introduction}

[01-kernel](../../01-kernel/) の「ファイルシステム」で、カーネルがファイルの読み書きを管理していると学びました
...
- [ファイルシステムの「なぜ」](#why-filesystem)
- [ファイルの正体：inode](#file-identity-inode)
- [ディレクトリエントリ：名前とinodeの橋渡し](#directory-entry)
- [ブロック：実際のデータの保存場所](#block-data-storage)
- [VFS：なぜ抽象化が必要なのか](#vfs-why-abstraction)
- [ファイルを開くとき何が起きるか](#what-happens-when-opening-file)
- [ハードリンクとシンボリックリンク](#hard-vs-symbolic-link)
...
## [ファイルシステムの「なぜ」](#why-filesystem) {#why-filesystem}

### [なぜファイルという概念が生まれたのか](#why-file-concept-was-born) {#why-file-concept-was-born}
...
### [なぜ階層構造（ディレクトリ）が必要なのか](#why-hierarchy-needed) {#why-hierarchy-needed}
...
## [ファイルの正体：inode](#file-identity-inode) {#file-identity-inode}

### [inodeとは](#what-is-inode) {#what-is-inode}
...
### [inodeが持つ情報](#inode-information) {#inode-information}
...
### [なぜファイル名はinodeに含まれないのか](#why-filename-not-in-inode) {#why-filename-not-in-inode}
...
## [ディレクトリエントリ：名前とinodeの橋渡し](#directory-entry) {#directory-entry}

### [ディレクトリエントリとは](#what-is-directory-entry) {#what-is-directory-entry}
...
### [ディレクトリは「ファイル」である](#directory-is-a-file) {#directory-is-a-file}
...
### [`.` と `..` の意味](#dot-and-dotdot-meaning) {#dot-and-dotdot-meaning}
...
### [名前解決の仕組み](#name-resolution-mechanism) {#name-resolution-mechanism}
...
## [ブロック：実際のデータの保存場所](#block-data-storage) {#block-data-storage}

### [ブロックとは](#what-is-block) {#what-is-block}
...
### [inodeとブロックの関係](#inode-and-block-relationship) {#inode-and-block-relationship}
...
### [なぜブロック単位で管理するのか](#why-block-management) {#why-block-management}
...
[page](../page/) で学んだように、メモリはページ（4KB）単位で管理されます

ブロックサイズをページサイズと一致させることで、ディスクからメモリへの転送が効率的になります

---

## [VFS：なぜ抽象化が必要なのか](#vfs-why-abstraction) {#vfs-why-abstraction}

### [ファイルシステムの多様性](#filesystem-diversity) {#filesystem-diversity}
...
### [VFS（仮想ファイルシステム）の役割](#vfs-role) {#vfs-role}
...
### [VFSの構造](#vfs-structure) {#vfs-structure}
...
## [ファイルを開くとき何が起きるか](#what-happens-when-opening-file) {#what-happens-when-opening-file}

### [open()の内部動作](#open-internal-operation) {#open-internal-operation}
...
### [ファイルディスクリプタとの関係](#file-descriptor-relationship) {#file-descriptor-relationship}
...
## [ハードリンクとシンボリックリンク](#hard-vs-symbolic-link) {#hard-vs-symbolic-link}

### [なぜリンクが必要なのか](#why-links-needed) {#why-links-needed}
...
### [ハードリンクの仕組み](#hard-link-mechanism) {#hard-link-mechanism}
...
### [シンボリックリンクの仕組み](#symbolic-link-mechanism) {#symbolic-link-mechanism}
...
### [両者の違いと使い分け](#differences-and-usage) {#differences-and-usage}
...
### [inodeの確認](#checking-inode) {#checking-inode}
...
### [ブロックの確認](#checking-block) {#checking-block}
...
### [リンクの確認](#checking-link) {#checking-link}
...
- [inode(7) - Linux manual page](https://man7.org/linux/man-pages/man7/inode.7.html){:target="\_blank"}
  - inode の詳細な説明
- [stat(2) - Linux manual page](https://man7.org/linux/man-pages/man2/stat.2.html){:target="\_blank"}
  - ファイルのメタデータ取得
...
- [Overview of the Linux Virtual File System - kernel.org](https://www.kernel.org/doc/html/latest/filesystems/vfs.html){:target="\_blank"}
  - VFS の公式ドキュメント
