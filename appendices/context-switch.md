---
layout: default
title: CPUは複数のプロセスをどう切り替えるのか
---

# [CPUは複数のプロセスをどう切り替えるのか](#how-cpu-switches-processes) {#how-cpu-switches-processes}

## [はじめに](#introduction) {#introduction}

[05-scheduler](../../05-scheduler/) では、カーネルのスケジューラが CPU 時間をプロセスに配分する仕組みを学びました
...
- [コンテキストとは](#what-is-context)
- [コンテキストスイッチの仕組み](#context-switch-mechanism)
- [自発的と非自発的](#voluntary-and-involuntary)
- [コンテキストスイッチのオーバーヘッド](#context-switch-overhead)
- [コンテキストスイッチの確認方法](#checking-context-switches)
- [まとめ](#summary)
- [参考資料](#references)

---

## [コンテキストとは](#what-is-context) {#what-is-context}

### [プロセスの「状態」](#process-state) {#process-state}
...
### [コンテキストに含まれる情報](#context-information) {#context-information}
...
### [なぜコンテキストを保存するのか](#why-save-context) {#why-save-context}
...
## [コンテキストスイッチの仕組み](#context-switch-mechanism) {#context-switch-mechanism}

### [切り替えの流れ](#switching-flow) {#switching-flow}
...
### [より詳細な流れ](#detailed-flow) {#detailed-flow}
...
### [ページテーブルの切り替え](#page-table-switching) {#page-table-switching}
...
TLB（Translation Lookaside Buffer）は、ページテーブルの検索結果をキャッシュする CPU 内部の高速メモリです（詳細は [page.md](../page/#tlbとは) を参照）
...
## [自発的と非自発的](#voluntary-and-involuntary) {#voluntary-and-involuntary}

### [2種類のコンテキストスイッチ](#two-types-of-context-switch) {#two-types-of-context-switch}
...
### [自発的コンテキストスイッチ](#voluntary-context-switch) {#voluntary-context-switch}
...
### [非自発的コンテキストスイッチ](#involuntary-context-switch) {#involuntary-context-switch}
...
## [コンテキストスイッチのオーバーヘッド](#context-switch-overhead) {#context-switch-overhead}

### [何がコストになるのか](#what-is-the-cost) {#what-is-the-cost}
...
### [TLBフラッシュの影響](#tlb-flush-impact) {#tlb-flush-impact}
...
### [CPUキャッシュの影響](#cpu-cache-impact) {#cpu-cache-impact}

切り替え後のプロセスは、前のプロセスのデータで埋まった CPU キャッシュを使います

必要なデータがキャッシュにないため、メモリアクセスが遅くなります

### [オーバーヘッドの目安](#overhead-estimate) {#overhead-estimate}
...
## [コンテキストスイッチの確認方法](#checking-context-switches) {#checking-context-switches}
...
### [vmstat コマンド](#vmstat-command) {#vmstat-command}
...
### [perf コマンド](#perf-command) {#perf-command}
...
- [vmstat(8) - Linux manual page](https://man7.org/linux/man-pages/man8/vmstat.8.html){:target="\_blank"}
  - システム統計の表示
- [perf(1) - Linux manual page](https://man7.org/linux/man-pages/man1/perf.1.html){:target="\_blank"}
  - パフォーマンス分析ツール

<strong>本編との関連</strong>

- [02-syscall](../../02-syscall/)
  - システムコールとモード切り替え
