...
# [02-syscall：システムコールの仕組み](#syscall-mechanism) {#syscall-mechanism}
...
1. [システムコールとは何か](#what-is-syscall)
2. [システムコールの仕組み](#syscall-mechanism)
3. [システムコールの呼び出し方](#how-to-call-syscall)
4. [ライブラリ関数とシステムコールの違い](#library-vs-syscall)
5. [errno によるエラー処理](#error-handling-with-errno)
6. [strace によるシステムコールの観察](#syscall-observation-with-strace)
7. [システムコールのオーバーヘッド](#syscall-overhead)
8. [vDSO による最適化](#optimization-with-vdso)
9. [ptrace によるシステムコールの傍受](#syscall-interception-with-ptrace)
...
## [システムコールとは何か](#what-is-syscall) {#what-is-syscall}
...
### [システムコールが必要な理由](#why-syscall-needed) {#why-syscall-needed}
...
### [システムコールの種類](#syscall-types) {#syscall-types}
...
## [システムコールの仕組み](#syscall-mechanism) {#syscall-mechanism}

### [モード切り替え](#mode-switch) {#mode-switch}
...
### [x86_64 でのシステムコール](#syscall-on-x86-64) {#syscall-on-x86-64}
...
## [システムコールの呼び出し方](#how-to-call-syscall) {#how-to-call-syscall}

### [方法 1：ライブラリ関数を使う（推奨）](#method-library-function) {#method-library-function}
...
### [方法 2：syscall() を使う](#method-syscall-function) {#method-syscall-function}
...
## [ライブラリ関数とシステムコールの違い](#library-vs-syscall) {#library-vs-syscall}

### [関係性](#relationship) {#relationship}
...
### [違いのまとめ](#comparison-summary) {#comparison-summary}
...
### [具体例：printf() と write()](#example-printf-and-write) {#example-printf-and-write}
...
## [errno によるエラー処理](#error-handling-with-errno) {#error-handling-with-errno}

### [なぜエラー処理が必要なのか](#why-error-handling) {#why-error-handling}
...
### [errno とは](#what-is-errno) {#what-is-errno}
...
### [エラー処理の基本パターン](#basic-error-handling-pattern) {#basic-error-handling-pattern}
...
### [よく使うエラー番号](#common-error-codes) {#common-error-codes}
...
### [errno の注意点](#errno-notes) {#errno-notes}
...
## [strace によるシステムコールの観察](#syscall-observation-with-strace) {#syscall-observation-with-strace}

### [strace とは](#what-is-strace) {#what-is-strace}
...
### [便利なオプション](#useful-options) {#useful-options}
...
### [strace の出力の読み方](#reading-strace-output) {#reading-strace-output}
...
## [システムコールのオーバーヘッド](#syscall-overhead) {#syscall-overhead}

### [オーバーヘッドとは](#what-is-overhead) {#what-is-overhead}
...
### [オーバーヘッドを減らす方法](#reducing-overhead) {#reducing-overhead}
...
## [vDSO による最適化](#optimization-with-vdso) {#optimization-with-vdso}

### [なぜ vDSO が必要なのか](#why-vdso-needed) {#why-vdso-needed}
...
### [vDSO とは](#what-is-vdso) {#what-is-vdso}
...
### [vDSO の効果](#vdso-effect) {#vdso-effect}
...
### [vDSO の仕組み](#vdso-mechanism) {#vdso-mechanism}
...
### [vDSO の確認方法](#checking-vdso) {#checking-vdso}
...
## [ptrace によるシステムコールの傍受](#syscall-interception-with-ptrace) {#syscall-interception-with-ptrace}

### [なぜ ptrace が必要なのか](#why-ptrace-needed) {#why-ptrace-needed}
...
### [ptrace とは](#what-is-ptrace) {#what-is-ptrace}
...
### [ptrace の用途](#ptrace-uses) {#ptrace-uses}
...
### [ptrace の主要な操作](#ptrace-main-operations) {#ptrace-main-operations}
...
次の [03-virtual-memory](../03-virtual-memory/) では、カーネルがプロセスごとにメモリ空間を分離する「仮想メモリ」の仕組みを学びます
...
- [syscall(2) - Linux manual page](https://man7.org/linux/man-pages/man2/syscall.2.html){:target="\_blank"}
  - syscall() 関数の詳細
- [syscalls(2) - Linux manual page](https://man7.org/linux/man-pages/man2/syscalls.2.html){:target="\_blank"}
  - Linux システムコールの一覧
...
- [strace(1) - Linux manual page](https://man7.org/linux/man-pages/man1/strace.1.html){:target="\_blank"}
  - strace コマンド
- [ptrace(2) - Linux manual page](https://man7.org/linux/man-pages/man2/ptrace.2.html){:target="\_blank"}
  - ptrace システムコール

<strong>最適化</strong>

- [vdso(7) - Linux manual page](https://man7.org/linux/man-pages/man7/vdso.7.html){:target="\_blank"}
  - vDSO の詳細
