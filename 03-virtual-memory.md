...
# [03-virtual-memory：仮想メモリの仕組み](#virtual-memory-mechanism) {#virtual-memory-mechanism}

## [はじめに](#introduction) {#introduction}

[02-syscall](../02-syscall/) では、ユーザー空間からカーネルへの「扉」であるシステムコールを学びました
...
1. [仮想メモリとは何か](#what-is-virtual-memory)
2. [アドレス空間の構造](#address-space-structure)
3. [ページングの仕組み](#paging-mechanism)
4. [/proc/self/maps でメモリマップを観察](#observing-memory-map)
5. [mmap() によるメモリマッピング](#memory-mapping-with-mmap)
6. [メモリ保護（mprotect）](#memory-protection-mprotect)
7. [メモリのロック（mlock）](#memory-locking-mlock)
8. [/proc/self/smaps で詳細情報を取得](#detailed-info-with-smaps)
9. [次のステップ](#next-steps)
10. [用語集](#glossary)
11. [参考資料](#references)
...
## [仮想メモリとは何か](#what-is-virtual-memory) {#what-is-virtual-memory}

### [なぜ仮想メモリが必要なのか](#why-virtual-memory-needed) {#why-virtual-memory-needed}
...
### [仮想メモリの解決策](#virtual-memory-solution) {#virtual-memory-solution}
...
### [仮想アドレスと物理アドレス](#virtual-and-physical-address) {#virtual-and-physical-address}
...
## [アドレス空間の構造](#address-space-structure) {#address-space-structure}

プロセスの仮想アドレス空間は、いくつかの領域に分かれています

### [主要な領域](#main-regions) {#main-regions}
...
### [アドレス空間のレイアウト](#address-space-layout) {#address-space-layout}
...
### [各領域の詳細](#region-details) {#region-details}
...
## [ページングの仕組み](#paging-mechanism) {#paging-mechanism}
...
### [ページサイズの確認方法](#checking-page-size) {#checking-page-size}
...
### [デマンドページングの利点](#demand-paging-benefits) {#demand-paging-benefits}
...
ページングの詳しい仕組みは [appendices/page.md](../appendices/page/) で学べます

---

## [/proc/self/maps でメモリマップを観察](#observing-memory-map) {#observing-memory-map}

### [/proc/self/maps とは](#what-is-proc-self-maps) {#what-is-proc-self-maps}
...
### [出力形式](#output-format) {#output-format}
...
### [フィールドの説明](#field-description) {#field-description}
...
### [権限フィールドの形式](#permission-field-format) {#permission-field-format}
...
### [特殊領域名](#special-region-names) {#special-region-names}
...
### [コマンドでの確認](#checking-with-command) {#checking-with-command}
...
## [mmap() によるメモリマッピング](#memory-mapping-with-mmap) {#memory-mapping-with-mmap}

### [mmap() とは](#what-is-mmap) {#what-is-mmap}
...
### [mmap() の主な用途](#mmap-main-uses) {#mmap-main-uses}
...
### [mmap() のフラグ](#mmap-flags) {#mmap-flags}
...
### [保護属性](#protection-attributes) {#protection-attributes}
...
### [munmap() による解放](#releasing-with-munmap) {#releasing-with-munmap}
...
## [メモリ保護（mprotect）](#memory-protection-mprotect) {#memory-protection-mprotect}

### [なぜメモリ保護が必要なのか](#why-memory-protection) {#why-memory-protection}
...
### [mprotect() とは](#what-is-mprotect) {#what-is-mprotect}
...
### [使用例](#mlock-usage-example) {#mlock-usage-example}
...
### [保護違反と SIGSEGV](#protection-violation-and-sigsegv) {#protection-violation-and-sigsegv}
...
### [mprotect() の用途](#mprotect-uses) {#mprotect-uses}
...
## [メモリのロック（mlock）](#memory-locking-mlock) {#memory-locking-mlock}

### [mlock() とは](#what-is-mlock) {#what-is-mlock}
...
### [なぜメモリをロックするのか](#why-lock-memory) {#why-lock-memory}
...
## [/proc/self/smaps で詳細情報を取得](#detailed-info-with-smaps) {#detailed-info-with-smaps}

### [/proc/self/smaps とは](#what-is-proc-self-smaps) {#what-is-proc-self-smaps}

`/proc/self/smaps` は、`/proc/self/maps` の拡張版で、各メモリ領域の詳細な統計情報を提供します

### [主要なフィールド](#main-fields) {#main-fields}
...
### [RSS vs PSS](#rss-vs-pss) {#rss-vs-pss}
...
次の [04-process-management](../04-process-management/) では、カーネルがプロセスを管理する責務について学びます
...
- [mmap(2) - Linux manual page](https://man7.org/linux/man-pages/man2/mmap.2.html){:target="\_blank"}
  - メモリマッピング
- [munmap(2) - Linux manual page](https://man7.org/linux/man-pages/man2/munmap.2.html){:target="\_blank"}
  - マッピングの解除
- [mprotect(2) - Linux manual page](https://man7.org/linux/man-pages/man2/mprotect.2.html){:target="\_blank"}
  - メモリ保護属性の変更
- [mlock(2) - Linux manual page](https://man7.org/linux/man-pages/man2/mlock.2.html){:target="\_blank"}
  - メモリのロック
- [msync(2) - Linux manual page](https://man7.org/linux/man-pages/man2/msync.2.html){:target="\_blank"}
  - ファイルへの同期
- [getpagesize(2) - Linux manual page](https://man7.org/linux/man-pages/man2/getpagesize.2.html){:target="\_blank"}
  - ページサイズの取得
...
- [sysconf(3) - Linux manual page](https://man7.org/linux/man-pages/man3/sysconf.3.html){:target="\_blank"}
  - \_SC_PAGESIZE

<strong>カーネルドキュメント</strong>

- [The Linux Kernel documentation - Memory Management](https://www.kernel.org/doc/html/latest/admin-guide/mm/){:target="\_blank"}
  - カーネルのメモリ管理
