---
layout: default
title: read.kernel-space
---

# [read.kernel-space](#read-kernel-space) {#read-kernel-space}
...
## [このリポジトリの特徴](#features) {#features}

### [<strong>カーネルソースは読まない</strong>](#no-source-reading) {#no-source-reading}

Linux カーネルのソースコードは非常に複雑で、初学者が読み解くのは困難です

このリポジトリでは、カーネルの動作を「観察」することで理解します

### [<strong>観察の手段</strong>](#observation-tools) {#observation-tools}
...
### [<strong>説明モデルとして理解する</strong>](#mental-model) {#mental-model}

「カーネル内部で実際にどう実装されているか」ではなく、「カーネルは何をしていると考えればよいか」を学びます

---

## [「カーネル空間」とは何か](#what-is-kernel-space) {#what-is-kernel-space}

コンピュータの世界には、大きく分けて2つの「空間」があります

### [<strong>1. ユーザー空間（User Space）</strong>](#user-space) {#user-space}

私たちが普段使うプログラム（アプリ）が動いている場所です

前のシリーズで学んだ内容は、すべてここに関するものでした

### [<strong>2. カーネル空間（Kernel Space）</strong>](#kernel-space) {#kernel-space}
...
## [なぜこれを学ぶのか](#why-learn) {#why-learn}

### [<strong>1. ブラックボックスを開く</strong>](#open-black-box) {#open-black-box}
...
### [<strong>2. コンテナ技術の基盤を理解する</strong>](#understand-container-foundation) {#understand-container-foundation}
...
### [<strong>3. トラブルシューティングの基礎</strong>](#troubleshooting-basics) {#troubleshooting-basics}
...
| 順番 | トピック | 学ぶこと |
| ---- | ------------------------------------------------ | --------------------------------------------------- |
| 01 | [kernel](./01-kernel/) | カーネルの役割、/proc と /sys で情報を観察する方法 |
| 02 | [syscall](./02-syscall/) | システムコールの仕組み、strace による観察 |
| 03 | [virtual-memory](./03-virtual-memory/) | 仮想メモリ、ページング、mmap によるメモリマッピング |
| 04 | [process-management](./04-process-management/) | プロセスの状態管理、プロセスグループ、デーモン |
| 05 | [scheduler](./05-scheduler/) | スケジューリングポリシー、nice 値、CFS |
| 06 | [namespace](./06-namespace/) | namespace によるプロセス隔離（コンテナの基盤） |
| 07 | [cgroup](./07-cgroup/) | CPU・メモリ・I/O のリソース制限（コンテナの基盤） |
...
## [前のシリーズとの関係](#relationship-with-previous-series) {#relationship-with-previous-series}
